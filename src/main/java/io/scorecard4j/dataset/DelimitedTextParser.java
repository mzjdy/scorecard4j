...
package io.scorecard4j.dataset;
...
public class DelimitedTextParser {
    ...
    /**
     * detectors for abnormal values
     */
    private Map<Integer, List<AbnormalDetector>> abnormalDetectors = new HashMap<Integer, List<AbnormalDetector>>();
    ...
    /**
     * Add {@link AbnormalDetector} to this parser
     * 
     * @param featureIdx
     *            0-based index of feature
     * @param detector
     *            {@link AbnormalDetector} for the feature at given index
     */
    public void addDectector(int featureIdx, AbnormalDetector detector) {
        List<AbnormalDetector> detectors = abnormalDetectors.get(featureIdx);
        if(detectors == null) {
           detectors = new ArrayList<AbnormalDetector>();
           abnormalDetectors.put(featureIdx, detectors);           
        }
    ...
    private AttributeDataset parse(String name, Attribute[] attributes, BufferedReader reader) throws IOException, ParseException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            int abnormalValueDetected = -1;
            rowCnt++;
            for (int i = 0, k = 0; i < s.length; i++) {
                if (!ignoredColumns.contains(i)) {
                    List<AbnormalDetector> detectors = abnormalDetectors.get(i);
                    if (i == responseIndex) {
                        y = response.valueOf(s[i]);
                    } else if (missing != null && missing.equalsIgnoreCase(s[i])) {
                        x[k++] = Double.NaN;
                        if (detectors != null) {
                            for(AbnormalDetector detector : detectors) {
                                if(detector.isAbnormal(Double.NaN) > 0) {
                                    abnormalValueDetected = i;
                                    break;                                    
                                }
                            }
